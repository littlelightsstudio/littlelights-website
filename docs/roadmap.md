...
**Letzte Aktualisierung: 15. Mai 2026 (Session 29: Bilinguale Auslieferung DE/EN komplett im Code — Path-Prefix-Routing, Middleware mit Cookie + Accept-Language + x-locale-Header, LocaleSwitcher in Desktop-Nav + Mobile-Drawer, dynamisches `<html lang>`, hreflang + canonical auf allen Page-Types, Dictionary mit `t()` + `categoryLabel()` ersetzt 50+ hardcoded Strings, Categories-DRY-Fix über 5 Files, interne Links komplett locale-aware. Resend-Kontaktformular auf nächste Session verschoben — DNS für `send.littlelights.studio` war noch nicht verifiziert.)**
...
2. **Mega-Menü + Footer EN-Content im lokalen CMS** (User-Aufgabe, kein Code) — Lokales DB-Setup hat nur DE im Header-Global und Footer-Global, deshalb fällt das Mega-Menü auf `/en/…` auf DE zurück (Fallback funktioniert sauber). Staging hat EN bereits gepflegt. Strategie: entweder manuell im Admin oder DB-Dump von Staging.
