...
- [x] **Zweisprachigkeit DE/EN** (Session 29, 15.05.) — Path-Prefix-Routing (`/en/…`, DE kanonisch ohne Prefix), Middleware mit Cookie + Accept-Language + 307+no-store gegen Cache-Probleme + `x-locale`-Header für Layout. LocaleSwitcher (DE/EN-Text-Toggle) Desktop in Nav zwischen Meta-Links + CTA, Mobile in Drawer. Fetch-Layer in allen Pages, Globals (Header, Footer, SiteSettings), Components (Nav, Footer, FeaturedProjects, Testimonials, Stats, TeamGrid) mit `locale` + `fallbackLocale: 'de'`. Dictionary [`src/lib/dict.ts`](../../littlelights-build/src/lib/dict.ts) mit `t(locale, key)` + `categoryLabel(locale, slug)` ersetzt 50+ hardcoded Strings (Project-Detail Section-Headlines, Case-Study-Labels inkl. Creative-Studio-Narrative „Der Funke/Die Reise/Das Werk" → „The Spark/The Journey/The Work", Categories inkl. Imagefilm → Image Film, Werbespot → Commercial, Form-Placeholders, Aria-Labels, Error-Titles, ProcessTimeline Loop-Indicator „wiederholt monatlich" → „repeats monthly" + Optional-Badge). Categories-DRY-Fix über 5 Files. hreflang + canonical via [`buildAlternates`](../../littlelights-build/src/lib/hreflang.ts) auf allen Page-Types (Homepage, Pages, Projects-Listing, Project-Detail). `<html lang>` dynamisch via Middleware-Header. Interne Links (Nav, Mega-Menü-Sub-Items, Footer-Spalten + -Items + Legal, Project-Tiles, Related, „Alle Projekte ansehen") nutzen `pathForLocale(locale, path)`. Slug-Strategie: Option A (gleiche DE-Slugs für beide Locales). Cookie-Banner nicht nötig — Cookie nur bei expliziter User-Aktion, fällt unter TTDSG §25 Abs. 2 Nr. 2. **Plus** [`scripts/locale-gaps.py`](../../littlelights-build/scripts/locale-gaps.py): Gap-Scanner geht alle Pages durch (locale=all), findet localized Felder mit DE gesetzt + EN null/empty, reportet als Markdown-Liste pro Page-Slug. Verwendung gegen Staging: `LL_BASE_URL=https://staging.littlelights.studio python3 scripts/locale-gaps.py`.
...
- [x] **P0-1 · `sitemap.xml` fehlt** — Session 30: `src/app/sitemap.ts` dynamisch aus Payload, 58 URLs, hreflang DE/EN/x-default, depth=3 für Parent-Chain.
- [x] **P0-2 · `robots.txt` fehlt** — Session 30: `src/app/robots.ts` host-aware (staging Disallow:/, prod normal mit Sitemap-Link).
- [x] **P0-3 · Title-Duplikat-Bug** — Session 30: payload-config `generateTitle` ohne Suffix + `[...slug]/page.tsx` strip-then-rebuild-Logik mit `alreadyHasBrand`-Check + `projects/page.tsx` mit `title.absolute`. Verifiziert 0× auf allen geprüften Pages.
- [x] **P0-4 · Organization-Schema fehlt global** — Session 30: `<OrganizationSchema />` in Layout, JSON-LD aus SiteSettings (name, legalName, email, phone, address, sameAs via social[]), `unstable_cache` + `revalidateTag` hook in SiteSettings.afterChange. Live mit LinkedIn + Instagram nach User-Pflege.
- [ ] **P0-5 · Image-Alt-Texte flächendeckend fehlend** — API-Sample 200/422 Media: **86 ohne Alt (43%)**. Required-Flag in Media-Collection bereits gesetzt seit Schema-Init, aber Bestandsdaten bypassed validation beim Import. Session 30 Pass 3 Plan: Backfill-Script analog zu `locale-gaps.py` → Liste der fehlenden Alts, manuelle Pflege im Admin durch User. 5 min Script, 2-4h Pflege.

##### ⚠️ P1 — Vor Launch hart fixen

- [x] **P1-1 · BreadcrumbList-Schema** — Session 30: `src/lib/breadcrumbs.ts` mit `buildPageCrumbs` (walks Parent-Chain via depth=5) + `buildProjectCrumbs`. `<BreadcrumbSchema />` rendered in `[...slug]/page.tsx` und `projects/[slug]/page.tsx`. Verifiziert auf staging.
- [x] **P1-2 · VideoObject-Schema auf Project-Detail** — Session 30: `<VideoObjectSchema />` mit Bunny-Embed-URL-Resolution (GUID-Regex → `iframe.mediadelivery.net/embed/639133/{guid}`), Thumbnail absolute, uploadDate aus createdAt, publisher als Org-Ref. Auto-Render wenn `project.mainVideo` gesetzt.
- [x] **P1-3 · Service-Schema auf Disziplin-Hubs** — Session 30: `<ServiceSchema />` Auto-Detection auf Pages mit `area` + `parent` gesetzt (Hubs ausgenommen). areaServed = Wien/Österreich/DACH, provider Org-Ref.
- [x] **P1-4 · OG-Image-Fallback Homepage + Listing** — Session 30: Layout-Metadata mit Default `/images/hero-allianz.png` (1920×1080). Pages mit eigenem ogImage überschreiben automatisch. CMS-managed Default ist späteres Polish-Item.
- [x] **P1-5 · Twitter-Cards Konsistenz** — Session 30: globaler Default `twitter.card = summary_large_image` in `layout.tsx`. `[...slug]/page.tsx` und `projects/page.tsx` setzen zusätzlich explizite title+description.
- ~~**P1-6 · 301-Redirects für alte Slugs**~~ — Session 30 User-Entscheidung: nur eine URL muss gerettet werden, `littlelights.studio/publishing` (gedruckt in Helena-Flinn-Büchern). Gelöst über `PATH_ALIASES` in `src/middleware.ts` — sichtbare URL bleibt `/publishing`, interner Rewrite zu `/creative-studio/publishing`, canonical zeigt auf die kanonische URL → kein Duplicate-Content. Alle anderen alten WordPress-Slugs sind nicht in Print-Material → egal wenn sie nach Cutover 404 returnen.
...
1. **Resend-Kontaktformular** (Session 30) — API-Route POST /api/contact, Resend SDK, Spam-Schutz (Honeypot + Rate-Limit, evtl. Cloudflare Turnstile), Zod-Validation, Success/Error-States, Email-Body locale-aware (Form weiß jetzt um Locale). DNS für `send.littlelights.studio` ist eingerichtet (Hetzner-Zone: MX + SPF + DKIM), Resend-Domain-Verifikation läuft. Form selbst ist seit Session 15 CMS-driven, nur Submit-Handler fehlt.
2. **CMS-EN-Gaps schließen** (User-Aufgabe, kein Code) — Scanner-Output gibt vollständige Liste: `LL_BASE_URL=https://staging.littlelights.studio python3 scripts/locale-gaps.py > /tmp/gaps.md`. Stand 15.05.: 476 Gaps lokal, davon ~95% Image-Alt-Texte + Episode-Titles. Sichtbar-relevant: Pricing-Tier-CTAs (Workshop schon erledigt: „Let's talk"), Legal-Pages (Impressum/Datenschutz/AGB könnten kosmetisch übersetzt werden, sind aber AT-rechtsbedingt DE-erlaubt), Hub-Page-Image-Alts. Mega-Menü + Footer-Global EN auf Staging bereits gepflegt.
3. **Restliche Mobile-Items** — iPad Mega-Menu-Position (820px-Portrait), Book-Reviews auf Mobile, 404-Page-Check.
